...
const ProductCard: FC<Props> = ({
		...
		return (
			...
			>
				...
				<div className='mt-12 text-white md:max-w-[270px]'>
					<h3 className='font-black text-4xl leading-[48px] line-clamp-2'>{product.name}</h3>
					<p className='mt-4 font-medium text-lg line-clamp-3'>{product.description}</p>
				</div>
				<Link
	...
	return (
		<Link className={className} href={`/product/${product.slug}`} aria-label={product.name}>
			{variant === 'simple' && (
				<>
			...
			{variant === 'default' && (
				<>
					{process.env.COMMERCE_WISHLIST_ENABLED && (
						<WishlistButton productId={product.id} variant={product.variants[0] as any} />
					)}
